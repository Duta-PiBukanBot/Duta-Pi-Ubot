...
async def promote(event):
    ...
    if not user:
        return
    sadevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await sadevent.edit(NO_PERM)
    await sadevent.edit("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    sadevent = await edit_or_reply(event, "`Demoting...`")
...
async def _ban_person(event):
    ...
    sadevent = await edit_or_reply(event, "`hacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    sadevent = await edit_or_reply(event, "`Unbanning...`")
    try:
        ...
        await sadevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await sadevent.edit(f"**Error :**\n`{e}`")


@tod.tod_cmd(incoming=True)
    ...
    if event.is_private:
        ...
        try:
            ...
            if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None:
                ...
                return await edit_or_reply(
                    event, "`You can't mute a person without admin rights niqq"
                )
...
async def endmute(event):
    ...
    if not user:
        return
    sadevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await sadevent.edit(NO_PERM + f"\n{e}")
    if reason:
        await sadevent.edit(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await sadevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    sadevent = await edit_or_reply(event, "`Searching recent actions .....`")
    ...
    if not flag:
        ...
        await edit_or_reply(sadevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(sadevent, deleted_msg)
        for msg in adminlog:
            ruser = (
                await event.client(GetFullUserRequest(msg.old.from_id.user_id))
            ).user
